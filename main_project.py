...
def create_screen():
    ...
    while bomb_counter <20:
        x = random.randrange(4, 47,4)
        y = random.randrange(1, 23, 2)
...
def main():
    ...
    person_image = pygame.image.load('soldier_img.png')
    person_image = pygame.transform.scale(person_image, (square_size*2, square_size*4))
    person_location = [0, 0]
    enter_pressed = False
    while True:
                ...
                if event.key == pygame.K_RETURN:
                    ...
                    for row in range(len(screen_squares)):
                        for col in range(len(screen_squares[row])):
                            pygame.draw.rect(surface, (0, 128, 0), screen_squares[row][col][0], 1)
                    for i in range(len(screen_squares)):
                        for j in range(len(screen_squares[i])):
                            if screen_squares[i][j][1]:
                                surface.blit(mine(),(i*20,j*20))
                                person_image = pygame.image.load('soldier_nigth.png')
                                person_image = pygame.transform.scale(person_image, (square_size * 2, square_size * 4))
